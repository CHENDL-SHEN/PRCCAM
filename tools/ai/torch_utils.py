...
def L2_Loss(A_tensors, B_tensors):
    return torch.pow(A_tensors - B_tensors, 2)  # 差的平方
...
def make_cam(x, epsilon=1e-5):
    # relu(x) = max(x, 0)
    x = F.relu(x)  # 8 6 16 16
    
    b, c, h, w = x.size()

    flat_x = x.view(b, c, (h * w))  # 相当于将图片从6通道16*16，展平为 6*256，变成二维了。
    max_value = flat_x.max(axis=-1)[0].view((b, c, 1, 1))  # 在每一个特征图上找到最大值。8 6 1 1max(axis=-1): 这表示最后一个维度取最大值，也就是256维度那里
    
    return F.relu(x - epsilon) / (max_value + epsilon)  # 也就相当于归一化
