...
class RandomResize:
    ...
    def __call__(self, image, mode=Image.BICUBIC):
        ...
        return image.resize(size, mode)  # resize是缩放的意思，mode是以什么插值方式进行缩放
...
class RandomResize_For_Segmentation:
    def __init__(self, min_image_size, max_image_size):
        ...
        self.modes = [Image.BICUBIC, Image.NEAREST]# BICUBIC 三次样条插值 NEAREST最近邻插值
    
    def __call__(self, data):
        image, mask = data['image'], data['mask']

        rand_image_size = random.randint(self.min_image_size, self.max_image_size)#在最小尺寸和最大尺寸之间随机生成一个图片尺寸
        
        w, h = image.size
        if w < h:
            scale = rand_image_size / h# scale缩放倍数
        else:
            scale = rand_image_size / w
        
        size = (int(round(w*scale)), int(round(h*scale)))# round四舍五入保留整数
        if size[0] == w and size[1] == h:
            pass
        else:
...
class RandomHorizontalFlip:
    ...
    def __call__(self, image):
        if bool(random.getrandbits(1)):  # bool（）中非0就为true，反之为false。随机生成布尔值，布尔值只可能是true或者false，random.getrandbits(k)随机生成0~[(2^k)-1],k=1即0~1
...
class RandomHorizontalFlip_For_Segmentation:
    ...
    def __call__(self, data):
        ...
        if bool(random.getrandbits(1)):# 随机获得一个0-1的布尔值
            data['image'] = image.transpose(Image.FLIP_LEFT_RIGHT)# 左右镜像处理
            data['mask'] = mask.transpose(Image.FLIP_LEFT_RIGHT)

        return data

class Normalize:
    def __init__(self, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):  # 就是把需要用到的变量赋给self.
        self.mean = mean
        self.std = std

    def __call__(self, image):  # 利用self.变量进行函数编写
        image = np.asarray(image)  # 变成三列那种数组形式
        norm_image = np.empty_like(image, np.float32)  # 建一个与"image"相同形状的、数据类型为np.float32的空数组，
...
class Normalize_For_Segmentation:
    ...
    def __call__(self, data):
        ...
        norm_image = np.empty_like(image, np.float32)# 生成和已有数组相同大小，类型的数组。
...
class RandomCrop:
    ...
    def get_random_crop_box(self, image):
        ...
        if w_space > 0:
            cont_left = 0
            img_left = random.randrange(w_space + 1)# +1相当于w_space也能随机生成，不+1则不会生成w_space
        else:
    ...
    def __call__(self, image, bbox_dic=None):
        ...
        cropped_image[dst_bbox['ymin']:dst_bbox['ymax'], dst_bbox['xmin']:dst_bbox['xmax']] = \
            image[src_bbox['ymin']:src_bbox['ymax'], src_bbox['xmin']:src_bbox['xmax']]# =\是回车的意思；就是把源接下来图放到512*512数组的dst位置处
