...
def save_QIMG(images):
    ...
    # 假设你有一个名为tensor的Tensor对象
    # 将Tensor转换为numpy数组
    numpy_array = images.cpu().detach().numpy()
    # 调整数组的形状以适应图像显示
    reshaped_array = np.transpose(numpy_array, (0, 2, 3, 1))

    # 循环遍历每个图像并保存为图片
    for i in range(16):
        image = reshaped_array[i]

        ori_image = np.empty_like(image, np.float32)
        ori_image[..., 0] = (image[..., 0] * imagenet_std[0] + imagenet_mean[0]) * 255.
        ori_image[..., 1] = (image[..., 1] * imagenet_std[1] + imagenet_mean[1]) * 255.
        ori_image[..., 2] = (image[..., 2] * imagenet_std[2] + imagenet_mean[2]) * 255.

        # 确保像素值在0-255之间
        clipped_array = np.clip(ori_image, 0, 255)
        # 转换数组的数据类型为uint8
        final_array = clipped_array.astype(np.uint8)
        plt.imsave(f'/media/ders/XS/SPCAM/experiments/TEST/IMG/image_{i}.png', final_array)
...
def poolfeat(input, prob, sp_h=16, sp_w=16):
    ...
    b, _, h, w = input.shape    # b=16, _=3, h=480, w=480
    h_shift_unit = 1
    w_shift_unit = 1
...
def pool_feat_2(probs, feats):
    ...
    b, cf, h, w = feats.shape    # feats(b,cf,h,w)=(144,3,10,10)
    probs = probs.view(b, cp, h*w).transpose(1, 2)      # probs(144,100,21)
    probs_sum = torch.sum(probs, dim=1, keepdim=True)   # probs_sum(144,1,21)
    feats = feats.view(b, cf, h*w)      # feats(144,3,100)
    ret = torch.bmm(feats, probs) / (probs_sum + 1e-5)  # ret(144,3,21)

    return ret

def up_feat_2(probs, feats_min):
    b, cp, h, w = probs.shape    # probs(b,cp,h,w)=(144,21,10,10)
    b, cf, cp = feats_min.shape  # feats_min(b,cf,cp)=(144,3,21)
    probs = probs.view(b, cp, h*w)      # probs(144,21,100)
    ret = torch.bmm(feats_min, probs).view(b, cf, h, w)     # ret(144,3,10,10)
