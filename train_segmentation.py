...
parser.add_argument('--min_image_size', default=128, type=int)
parser.add_argument('--max_image_size', default=384, type=int)
...
if __name__ == '__main__':
    ...
    # log_dir = create_directory(f'./experimentsI512s512_SFC/segmentation/logs/')  # 创建文件夹
    # data_dir = create_directory(f'./experimentsI512s512_SFC/segmentation/data/')
    # model_dir = create_directory('./experimentsI512s512_SFC/segmentation/models/')
    # tensorboard_dir = create_directory(f'./experimentsI512s512_SFC/segmentation/{args.tag}/')
    log_dir = create_directory('/media/ders/sdc1/FJM/115P4mul_ipc_AP_SAM/Generality/experimentsP256s256_crop4/segmentation/logs/')  # 创建文件夹
    data_dir = create_directory('/media/ders/sdc1/FJM/115P4mul_ipc_AP_SAM/Generality/experimentsP256s256_crop4/segmentation/data/')
    model_dir = create_directory('/media/ders/sdc1/FJM/115P4mul_ipc_AP_SAM/Generality/experimentsP256s256_crop4/segmentation/models/')
    tensorboard_dir = create_directory('/media/ders/sdc1/FJM/115P4mul_ipc_AP_SAM/Generality/experimentsP256s256_crop4/segmentation/' + f'{args.tag}')
    pred_dir = '/media/ders/sdc1/FJM/115P4mul_ipc_AP_SAM/Generality/experimentsP256s256_crop4/pseudo_labels/'
    ...
    data_dic = {
        'train' : [],
        'validation' : [],
    }# data_dic是一个字典，train和validation是键，用来存储训练和验证的数据
    ...
    # torch.autograd.set_detect_anomaly(True)# 正向传播时：开启自动求导的异常侦测；该功能可以帮助我们更容易地发现和定位导致自动求导过程出错的代码位置。
