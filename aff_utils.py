...
class PathIndex:
    def __init__(self, radius, default_size):
        self.radius = radius
        self.radius_floor = int(np.ceil(radius) - 1)  # 向上取整后再减去 1，并将结果转换为整数类型，然后将结果赋值给 self.radius_floor 这个属性

        self.search_paths, self.search_dst = self.get_search_paths_dst(self.radius)  # 邻域半圆上的点self.search_dst；
    ...
    def get_search_paths_dst(self, max_radius=5):
        ...
        search_dirs = []
        for x in range(1, max_radius):  # 找y轴上坐标
            search_dirs.append((0, x))

        for y in range(1, max_radius):
            for x in range(-max_radius + 1, max_radius):
                if x * x + y * y < max_radius ** 2:
                    search_dirs.append((y, x))  # 半圆上所有的点

        for dir in search_dirs:
            length_sq = dir[0] ** 2 + dir[1] ** 2  # 坐标点x平方+y平方
            path_coords = []

            min_y, max_y = sorted((0, dir[0]))  # 从小到大排序
            ...
            path_coords.sort(key=lambda x: -abs(x[0]) - abs(x[1]))  # 排序0变为03
    ...
    def get_path_indices(self, size):
        full_indices = np.reshape(np.arange(0, size[0] * size[1], dtype=np.int64), (size[0], size[1]))  # 存的是每一个像素的索引

        cropped_height = size[0] - self.radius_floor  # 去掉半径之后剩下的高度
        cropped_width = size[1] - 2 * self.radius_floor  # 去掉直径剩下的宽度，也就是剩下除了半圆的区域

        path_indices = []
        for paths in self.search_paths:  # self.search_paths相当于每一个点的搜索范围

            path_indices_list = []
            for p in paths:  # 对于每一个路径点执行操作
                coord_indices_list = []

                for dy, dx in p:  # dy和dx分别代表每一个像素点的坐标
                    coord_indices = full_indices[dy:dy + cropped_height,
                                    self.radius_floor + dx:self.radius_floor + dx + cropped_width]
                    coord_indices = np.reshape(coord_indices, [-1])  # 展平处理
